...
* We wired our launcher (2 motors and a continuous rotation servo) to the Pupper through 2 NMOS-controlled switches for the motors, and an Arduino-driven 1 ms 50Hz pulse for the servo. The circuit was driven through an input voltage fed through the left ear servo (pin 26 on the Pupper's pi) to the Arduino. Time constraints prevented the use of flyback diodes to prevent high voltage loads across the transistors, but they held up quite well for the demo. See the circuit diagram [here](./circuit_diagram.jpeg), and the Arduino code [here](./launcher_arduino.ino).
